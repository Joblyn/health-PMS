...
export default function PatientDocuments() {
  ...
  useEffect(() => {
    let endpoint = getDocumentsByCompanyCodeEndpoint + patient.companyCode;
      ...
      setDocumentsState(
        documents.data.filter(
          (item) =>
            ...
            item.Document.docType === "document"
        )
      );
  }, [documents]);
      ...
      setSearchItems(
        documentsState.filter(
          (item) =>
            ...
            item.Dietitian.name.includes(target.value)
        )
      );
    }
  };
                ...
                <tbody>
                  {searchItems.reverse().map((document, id) => {
                    ...
                    return (
                      <tr>
                        <td>{id + 1}</td>
                        <td>{document.Dietitian.name}</td>
                        <td>{document.Document.docName}</td>
                        <td>
